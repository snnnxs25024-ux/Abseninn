...
const fixIcons = (filePath) => {
    ...
    content = content.replace(/import [a-zA-Z]+Icon from '\.\/components\/icons\/[a-zA-Z]+Icon';\n/g, '');
    content = content.replace(/import [a-zA-Z]+Icon from '\.\/icons\/[a-zA-Z]+Icon';\n/g, '');
    ...
    if (content.match(/<DashboardIcon/)) iconsToImport.add('LayoutDashboard');
    if (content.match(/<AttendanceIcon/)) iconsToImport.add('CalendarCheck');
    if (content.match(/<DatabaseIcon/)) iconsToImport.add('Database');
    if (content.match(/<LinkIcon/)) iconsToImport.add('Link');
    if (content.match(/<SettingsIcon/)) iconsToImport.add('Settings');
    if (content.match(/<LogoutIcon/)) iconsToImport.add('LogOut');
    if (content.match(/<SoundOnIcon/)) iconsToImport.add('Volume2');
    if (content.match(/<SoundOffIcon/)) iconsToImport.add('VolumeX');
    ...
    content = content.replace(/<DashboardIcon[^>]*>/g, '<LayoutDashboard size={20} />');
    content = content.replace(/<AttendanceIcon[^>]*>/g, '<CalendarCheck size={20} />');
    content = content.replace(/<DatabaseIcon[^>]*>/g, '<Database size={20} />');
    content = content.replace(/<LinkIcon[^>]*>/g, '<Link size={20} />');
    content = content.replace(/<SettingsIcon[^>]*>/g, '<Settings size={20} />');
    content = content.replace(/<LogoutIcon[^>]*>/g, '<LogOut size={20} />');
    content = content.replace(/<SoundOnIcon[^>]*>/g, '<Volume2 size={24} className="text-blue-500" />');
    content = content.replace(/<SoundOffIcon[^>]*>/g, '<VolumeX size={24} className="text-gray-400" />');
